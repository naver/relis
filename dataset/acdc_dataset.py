...
class ACDC(data.Dataset):
	def __init__(self, root, scene_list, cond_list, batch_size = 1, num_epochs=1):
		...
		for scene, cond in zip(self.scene_list, self.cond_list):
			...
			self.img_paths = glob.glob(os.path.join(self.images_root, 'rgb_anon_trainvaltest/rgb_anon', cond, 'train', scene, '*.png'))
			self.img_paths += glob.glob(os.path.join(self.images_root, 'rgb_anon_trainvaltest/rgb_anon', cond, 'val', scene, '*.png'))
			self.img_paths = sorted(self.img_paths)

			self.label_img_paths = [os.path.join(self.root, 'gt_trainval/gt', cond, path.split('/')[-3], scene,
