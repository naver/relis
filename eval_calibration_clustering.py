...
class SolverOps:
    ...
    def __init__(self, args):
        self.args = args
        if self.args.src_dataset == 'Cityscapes':
    ...
    def compute_calibration(self):
        ...
        """
        Compute calibration to temperature scaled images.
        Temperature is selected depending on the cluster.
        ...
        if len(best_temperatures) > 1:
            ...
            cluster_labels = np.zeros(all_logits.shape[0])
        
        print(f'Scaling logits according to cluster temperatures.')
        # Compute temperature scaled logits
        if self.args.cluster_assignment == 'soft':
            scaled_logits = self.scaled_logits_soft_assignment(all_logits, cluster_probs, best_temperatures)
        else:
            scaled_logits = self.scaled_logits(all_logits, cluster_labels, best_temperatures)
        
        scaled_logits, all_labels = self.reshape_logits_labels(scaled_logits, all_labels)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--trg_dataset_list", type=str, default='Cityscapes',
                        help="Which target datasets to transfer to")
    ...
    solver_ops.compute_calibration()
